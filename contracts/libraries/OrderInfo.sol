...
library OrderInfo {
    function isValidRequest(DataStructs.OrderInfo memory info, uint256 chainId) internal view returns(bool){
        return info.customer != address(0)
            && info.chainId > 0
            && info.chainId == chainId
            && info.tokenId > 0
            && info.numberOfYears > 0
            && info.openTime + info.openWindow > block.timestamp
            && info.nonce > 0;
    }
    ...
    function inPaymentWindow(DataStructs.OrderInfo memory info) internal pure returns(bool){
    ...
    function hasPayment(DataStructs.OrderInfo memory info) internal view returns(bool){
        if(info.paymentToken == address(0)){
            ...
            return IERC20(info.paymentToken).balanceOf(msg.sender) >= info.paymentAmount
    ...
    function lockPayment(DataStructs.OrderInfo memory info) internal returns(bool){
        if(info.paymentToken == address(0)){
            ...
            return IERC20(info.paymentToken).balanceOf(address(this) >= (balanceBefore + info.paymentAmount);
        }
    }
}
